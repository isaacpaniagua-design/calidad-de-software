...
// sin tocar tu disenio. Calcula KPIs, tabla "Mis calificaciones" y escala 0-5.
...
function normalizeItems(source) {
  ...
  for (let i = 0; i < source.length; i++) {
    ...
    const clampedPts = normalizedMax > 0 ? Math.max(0, Math.min(pts, normalizedMax)) : 0;
...
function scoreUnidad(arr) {
  ...
  return resumenGlobal(arr).porcentaje;
}
function final3040(u1, u2, u3) {
  return clampPct(u1 * 0.3 + u2 * 0.3 + u3 * 0.4);
}
...
function renderAlumno(items) {
  ...
  const { porcentaje, pondSum } = resumenGlobal(normalized);
  ...
  if (tbody) {
    ...
    if (!normalized.length) {
      tbody.innerHTML = '<tr><td class="qsc-muted" colspan="9">Sin actividades registradas aun.</td></tr>';
    } else {
      for (let i = 0; i < normalized.length; i++) {
        ...
        const fecha = (() => {
          try {
            const f = it.fecha;
            if (f && typeof f.toDate === 'function') return f.toDate().toLocaleDateString();
  ...
  if ($id('progressBar')) {
    ...
    pb.style.width = fin + '%';
    pb.className = 'h-3 rounded-full progress-bar';
    if (fin >= 90) pb.classList.add('bg-gradient-to-r', 'from-green-500', 'to-green-600');
    else if (fin >= 70) pb.classList.add('bg-gradient-to-r', 'from-yellow-500', 'to-yellow-600');
    else if (fin >= 60) pb.classList.add('bg-gradient-to-r', 'from-orange-500', 'to-orange-600');
    else pb.classList.add('bg-gradient-to-r', 'from-red-500', 'to-red-600');
  }
}
...
async function main() {
  ...
  if (root && root.dataset && root.dataset.grupo) grupoIdVal = root.dataset.grupo;
  else {
