...
function ready() {
  ...
  function resumenGlobal(items) {
    ...
    for (const it of items) {
      ...
      const clampedPts =
        normalizedMax > 0 ? Math.max(0, Math.min(pts, normalizedMax)) : pts;
  ...
  if ($id("progressPercent"))
    $id("progressPercent").textContent = String(Math.round(fin)) + "%";
  if ($id("progressBar")) {
    const bar = $id("progressBar");
    bar.style.width = fin + "%";
    bar.className = "h-3 rounded-full progress-bar";
    if (fin >= 90)
      bar.classList.add("bg-gradient-to-r", "from-green-500", "to-green-600");
    else if (fin >= 70)
      bar.classList.add("bg-gradient-to-r", "from-yellow-500", "to-yellow-600");
    else if (fin >= 60)
      bar.classList.add("bg-gradient-to-r", "from-orange-500", "to-orange-600");
...
async function obtenerItemsAlumno(db, grupoId, profile) {
  const { collection, doc, getDoc, getDocs, query, orderBy } = await import(
...
async function main() {
  ...
  onAuth(async (user) => {
    if (!user) {
      renderAlumno([]);
      setStatusMessage(
        "Inicia sesión con tu correo @potros.itson.edu.mx para ver tu progreso."
      );
      return;
    }
    try {
      ...
      renderAlumno(items);
      setStatusMessage(
        items && items.length ? "" : "Sin actividades registradas aún."
      );
    } catch (e) {
      ...
      if (
        ...
        setStatusMessage(
          "Tu cuenta no tiene permisos para consultar las calificaciones en línea. Contacta al docente para habilitar el acceso."
        );
      } else {
        setStatusMessage(
          "No se pudieron cargar tus calificaciones. Intenta nuevamente más tarde."
        );
      }
    }
  });
