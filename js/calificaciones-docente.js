...
async function loadTeacherGrades() {
  ...
  if (!user) {
    console.log(
      "Usuario no autenticado, no se pueden cargar las calificaciones del docente."
    );
    const teacherGradesEmpty = document.getElementById("teacherGradesEmpty");
    if (teacherGradesEmpty) {
      teacherGradesEmpty.textContent =
        "No se pudo verificar la autenticación para cargar las calificaciones.";
...
auth.onAuthStateChanged((user) => {
  if (user) {
    ...
    if (teacherGradesEmpty) {
      teacherGradesEmpty.textContent =
        "Necesitas iniciar sesión para ver las calificaciones.";
      teacherGradesEmpty.hidden = false;
    }
  }
});
