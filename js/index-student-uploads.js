...
import { onFirebaseReady, getAuthInstance } from "./firebase.js"; // <-- CORRECCIÓN: Usar el nuevo inicializador
...
document.addEventListener("DOMContentLoaded", () => {
  // Esperar a que Firebase esté listo antes de hacer nada.
  onFirebaseReady(() => {
    // --- Referencias al DOM (con validación) ---
    const form = document.getElementById("studentUploadForm");
    if (!form) {
      console.log(
        "El formulario de entrega de actividades no se encuentra en esta página. Script detenido."
      );
      return;
    }
    ...
    const auth = getAuthInstance(); // <-- CORRECCIÓN: Obtener instancia de Auth
    onAuthStateChanged(auth, (user) => {
      ...
      if (user) {
        ...
        defaultStatusMessage =
          "Debes iniciar sesión para poder registrar entregas.";
    ...
    function updateUploadStatus(message, type, doScroll = true) {
      ...
      uploadStatusDiv.classList.remove(
        "is-loading",
        "is-success",
        "is-error",
        "is-info"
    ...
    async function handleFormSubmit(e) {
      e.preventDefault();
      if (!currentUser)
        return updateUploadStatus("Error: No has iniciado sesión.", "error");
      ...
      if (!selectedOption || selectedOption.disabled)
        return updateUploadStatus(
          "Error: Debes seleccionar una actividad.",
          "error"
        );
      ...
      try {
        ...
        updateUploadStatus(
          "Registrando entrega en la plataforma...",
          "loading"
        );
        const payload = {
        ...
        updateUploadStatus(
          "¡Entrega exitosa! Tu archivo fue registrado.",
          "success"
        );
    ...
    function startObserver(uid) {
      observeStudentUploads(
        ...
        (error) => {
          ...
          updateUploadStatus(
            "Error al cargar tus entregas anteriores. Si el problema persiste, contacta a soporte.",
            "error"
          );
        }
      );
