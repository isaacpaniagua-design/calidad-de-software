...
async function loadStudentsFromFirestore() {
  ...
  try {
    ...
    studentsList = studentsList.filter(
      (student) => student.displayName && student.displayName.trim() !== ""
    );
    studentsList.sort((a, b) => a.displayName.localeCompare(b.displayName));

    studentSelect.innerHTML =
      '<option value="">-- Seleccione para ver --</option>';
    ...
    console.error(
      "Error definitivo cargando estudiantes desde Firestore:",
      error
    );
...
function renderStudentGrades(grades) {
  ...
  if (grades.length === 0) {
    activitiesContainer.innerHTML =
      '<p class="text-center text-gray-500 py-4">Este estudiante no tiene calificaciones asignadas.</p>';
    return;
  }
  grades.sort(
    (a, b) => (a.assignedAt?.toDate() || 0) - (b.assignedAt?.toDate() || 0)
  );
  grades.forEach((grade) => {
    const card = document.createElement("div");
    card.className =
      "activity-card bg-gray-50 p-4 rounded-lg shadow grid grid-cols-1 md:grid-cols-3 gap-4 items-center";
    card.innerHTML = `
            ...
            <div class="grid grid-cols-2 gap-2 items-center">
                ...
                <button data-action="delete-grade" data-grade-id="${
                  grade.id
                }" class="bg-red-100 text-red-700 text-xs py-2 px-2 rounded-md hover:bg-red-200">Eliminar</button>
            </div>
        `;
...
function loadGradesForStudent(studentId) {
  ...
  unsubscribeFromGrades = onSnapshot(
    ...
    (error) => {
      ...
      activitiesContainer.innerHTML =
        '<p class="text-center text-red-500 py-4">Error al cargar calificaciones.</p>';
    }
  );
}
