...
// Vista de estudiante para preview docente: popula #studentSelect y pinta tabla.
...
function ensureUI(root) {
  // Inserta el preview justo debajo del primer .qsc-wrap
...
function renderQsp(items) {
  const tbody = $id("qsp-tbody"),
    kpiT = $id("qsp-kpi-total"),
    kpiI = $id("qsp-kpi-items"),
    kpiP = $id("qsp-kpi-pond"),
    bar = $id("qsp-bar-fill");
...
async function resolverUidPorMatricula(db, matricula) {
  try {
    ...
    console.warn("[preview] resolverUidPorMatricula", e);
  }
  return null;
}
...
// Lista de alumnos para #studentSelect:
// 1) lista local window.students si existe
// 2) Firestore: courses/{grupo}/members con role=student
// 3) Firestore: grupos/{grupo}/members con role=student
// 4) Firestore: /users con role=student
async function fetchStudentList(db, grupoId) {
  // 1) local
  if (Array.isArray(window.students) && window.students.length) {
    return window.students.map((s) => ({
      uid: s.uid || null,
      matricula: s.id || s.matricula || null,
      displayName: s.name || s.displayName || "Alumno",
      email: s.email || "",
    }));
  }
  ...
  const out = [];
  const seen = new Set();
  async function pushFrom(qSnap, pathTag) {
    qSnap.forEach((d) => {
      const m = d.data() || {};
      const uid = d.id;
      ...
      if (!seen.has(key)) {
        seen.add(key);
        out.push({ uid, matricula, displayName, email });
      }
    });
  }
  ...
  try {
    const s1 = await getDocs(
      query(
        collection(db, `courses/${grupoId}/members`),
        where("role", "==", "student")
      )
    );
    if (!s1.empty) await pushFrom(s1, "courses");
  } catch (_) {}
  try {
    const s2 = await getDocs(
      query(
        collection(db, `grupos/${grupoId}/members`),
        where("role", "==", "student")
      )
    );
    if (!s2.empty) await pushFrom(s2, "grupos");
  } catch (_) {}
  if (!out.length) {
    try {
      ...
      if (!s3.empty) await pushFrom(s3, "users");
    } catch (_) {}
  }
  return out;
}
...
async function main() {
  ...
  if (!cfgOk) {
    console.warn("[preview] falta firebaseConfig");
    return;
  }
  ...
  if (sel) {
    ...
    for (const m of list) {
      ...
      const label = m.displayName
        ? `${m.displayName} · ${m.email || m.matricula || m.uid}`
        : m.email || m.matricula || m.uid || "Alumno";
    ...
    sel.addEventListener("change", async () => {
      ...
      const uid =
        opt?.dataset?.uid || (await resolverUidPorMatricula(db, matricula));
      if (!uid) {
        $id("qsp-tbody").innerHTML =
          '<tr><td class="qsc-muted" colspan="6">No se encontró UID para la selección.</td></tr>';
        return;
      }
      try {
        ...
        $id("qsp-tbody").innerHTML =
          '<tr><td class="qsc-muted" colspan="6">Error al cargar datos.</td></tr>';
      }
    });
  } else {
    console.warn("[preview] Falta #studentSelect en la página.");
  }
}
